...
The number of worker threads is configured with the `workflowThreadPoolSize` option. It defaults to 1 if `reuseV8Context` is true, which it most likely should be, or 2 otherwise.

Workflow tasks tend to be lightweight, so the defaults should be fine.
...
<div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 1rem 1.5rem; margin: 2rem 0; border-radius: 4px;">
    ...
    (they take ~10ms), but because there are many slots, which implies there are many pollers and workflow
    task completions that need to be sent to the Temporal server, which fills the event loop, blocking the
    main thread which delays the response to the Temporal server.
</div>

### Threading model

The tuner grants way too many workflow slots no matter what `targetCPU` is specified. To understand why, we need to look at the threading model.
...
I tested running 2 vCPU instances, with the default configuration, and the tuner was _more_ useful, but still granted too many slots.
...
When configuring a fixed sized workflow task slot supplier, with 2 vCPUs, I could get up to twice the number of slots as with 1 vCPU, but I was unable to reach more than 75% CPU usage, which makes sense, as performance is bound by the main thread, which is mostly I/O bound. \
Performance with 2 vCPUs is more stable, as variations in workflow task cost and having to replay or not happen in the worker thread, which is not at 100%.
...
<div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 1rem 1.5rem; margin: 2rem 0; border-radius: 4px;">
    <strong>Key Insight:</strong> Using resource-based tuner (the default) for workflow tasks with 1 vCPU
    makes `targetCPU` useless. With 2 vCPUs, it's still not good enough.
</div>

## Conclusions

I do not recommend using the tuner with default options, it causes too many workflow task slots, which makes them time out and be retried, burning CPU cycles for no reason.
...
It just is bad for the typescript sdk (I think the python sdk is similar), which only executes workflows on the worker thread, and the main thread is the limiting factor. It is even worse for 1 vCPU instances, which is how Node workers are usually deployed.

## Our Current worker configuration
...
Even though the default is not good, the docs are correct in that worker performance is mostly network bound so it's still useful to have multiple slots. We want to ensure that the worker thread is never idle due to having no work to do.
