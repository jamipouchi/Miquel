...
This article is related to worker configuration, but to run workers in production, infrastructure also needs to be considered.

Workers should run on fixed-size instances and scale horizontally. (We use ECS Fargate)

Scaling should be done on average across worker instances CPU usage, and either task backlog count or backlog age. (We send task queue metrics to CloudWatch)
